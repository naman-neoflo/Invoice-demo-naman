// components/spend-analytics/integration-card.tsx
//
// Card for a single integration entry on the spend-analytics Integrations
// surface. Active cards open a connection-details dialog; coming cards open a
// "notify me" dialog. Spec: docs/handoff/spend-analytics/03-screen-specs.md
// § "Surface 6: Integrations".
//
// TODO Phase 2: extract a shared <IntegrationCard> and refactor cash-app's,
// invoice-processing's, collections', and spend-analytics' integrations
// pages to share one component. For Phase 1 we keep them duplicated so the
// prior surfaces carry zero regression risk.
"use client"
...
export function IntegrationCard({ entry }: IntegrationCardProps) {
  ...
  function handleNotify() {
    ...
    console.log(
      `[spend-analytics] Notify ${notifyEmail} when ${entry.name} ships`
    )
  }
